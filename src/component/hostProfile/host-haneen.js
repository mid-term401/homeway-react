...
function HostProfileViewingAsHost(props) {
  ...
  useEffect(() => {
    console.log(
      "useEffect ~~~~ props.userData.loggedIn",
      state.thunkReducer.results.text
    );
    console.log(3, state.userData.loggedIn);
    if (state.userData.loggedIn) {
      // dispatch(
      //   bearerAuth(
      //     "https://robust-entity-homeway.herokuapp.com/host",
      //     state.userData.id,
      //     state.userData.cookie
      //   )
      // );
      // host = state;
      loadProfile();
  ...
  function loadProfile() {
    ...
    // results = JSON.parse(state.thunkReducer?.results?.text);
    // console.log(
